...
class FileCommands:
    ...
    async def mp3(self, message):
        ...
        if message.reply_to_message:
            ...
            try:
                ...
                await safe_call(message.reply_audio, audio=output_path, caption="🎵 Audio convertido desde video")
                await safe_call(progress_msg.delete)
            except Exception as e:
                await safe_call(progress_msg.edit_text, f"❌ Error: {str(e)}")
            finally:
    ...
    async def sendfile(self, message, user_id):
        ...
        if os.path.isfile(item_path):
            await self.bot._send_document_with_progress(message.chat.id, item_path, caption=f"📄 {selected_item}", user_id=user_id)
